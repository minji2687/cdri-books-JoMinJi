...
│
├── components/
│   ├── ui/                        # 공통 UI (Button, Input, icons)
│   ├── book/                      # 도서 카드 (BookCard, Compact, Expanded, List)
│   ├── search/                    # 검색바, 검색 기록, 상세 검색
│   └── layout/                    # Header, EmptyState
│
├── routes/                        # 페이지 라우트 (search, favorites)
├── hooks/                         # useSearchHistory
├── store/                         # Zustand 찜하기 스토어
└── types/                         # 공통 타입 정의
```
...
상세 검색 팝업에서 입력값 상태를 자식 컴포넌트가 아닌 부모 컴포넌트가 직접 관리합니다.

**왜 이렇게 했나?**
- 전체 검색을 하면 상세 검색 입력창을 초기화해야 하는데, 입력창이 자기만의 상태를 가지고 있으면 부모가 초기화할 방법이 없음
- 부모가 `query` 값을 props로 내려주고 변경도 부모가 처리하도록 만들면, 부모에서 언제든 값을 바꾸거나 초기화 가능

```typescript
// 부모 컴포넌트에서 상태를 가지고 있음
function SearchPage() {
  const [detailQuery, setDetailQuery] = useState('')
  
  const handleSearch = () => {
    setDetailQuery('')  // 부모가 직접 초기화 가능
  }
  
  return (
    <DetailSearchPopup 
      query={detailQuery}           // 값을 내려줌
      onQueryChange={setDetailQuery} // 변경 함수도 내려줌
    />
  )
}
```
...
**장점:**
- 상태가 한 곳에 있어서 흐름 추적이 쉬움
- 부모에서 자식 상태를 자유롭게 제어 가능
